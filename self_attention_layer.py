...
def self_attention(input_shape, prefix = "att", mask = False, **kwargs):
    """
        Self-Attention layers at transformer encoder and decoder.  Assumes its input is the oputput form the
        positional encosing layer. 

        Args:
            prefix (str): The prefix added to the layer names
            masked(bool): Whether to use casual mask. Should be Flase on encoder and True on
                          decoder. When True, a mask will be appled such that each location only has
                          access to the location before it. 
    """
    ...
    ##duncitonal API to connect input to output
...
tf.keras.utils.plot_model(model, "self_attention.png", show_shapes=True, show_dtype=True, show_layer_names=True,
