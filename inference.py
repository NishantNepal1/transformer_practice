...
from vectorisation_2 import test_pairs, eng_vectorizer, fre_vectorizer
from pos_en_layer import PositionalEmbedding
from custome_lr import CustomSchedule, masked_loss, masked_accuracy
...
def translate(sentence):
    ...
    lookup = list(fre_vectorizer.get_vocabulary())
    start_sentinel, end_sentinel = "[start]", "[end]"
    output_sentence = [start_sentinel]
    # generate the translated sentence word by word
    for i in range(seq_len):
        vector = fre_vectorizer([" ".join(output_sentence)])
